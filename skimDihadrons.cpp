...
int main(int argc, char** argv) {
  ...
  //// input banks
  auto b_event       = iguana::Algorithm::GetBankIndex(banks, "REC::Event");
  auto b_config      = iguana::Algorithm::GetBankIndex(banks, "RUN::config");
  auto b_particle    = iguana::Algorithm::GetBankIndex(banks, "REC::Particle");
  auto b_calorimeter = iguana::Algorithm::GetBankIndex(banks, "REC::Calorimeter");
  auto b_track       = iguana::Algorithm::GetBankIndex(banks, "REC::Track");
  auto b_traj        = iguana::Algorithm::GetBankIndex(banks, "REC::Traj");
  //// input MC banks
  auto b_mc_particle = useMCrec ? iguana::Algorithm::GetBankIndex(banks, "MC::Particle") : 0;
  auto b_mc_lund     = useMCrec ? iguana::Algorithm::GetBankIndex(banks, "MC::Lund") : 0;
  //// output banks
  auto b_inclusive_kin     = iguana::Algorithm::GetBankIndex(banks, "physics::InclusiveKinematics");
  auto b_dihadron_kin      = iguana::Algorithm::GetBankIndex(banks, "physics::DihadronKinematics");
  auto b_single_hadron_kin = iguana::Algorithm::GetBankIndex(banks, "physics::SingleHadronKinematics");
  ...
  Float_t W{0};     outrootTr->Branch("W",     &W,      "W/F");
  Float_t Q2{0};    outrootTr->Branch("Q2",    &Q2,     "Q2/F");
  Float_t Nu{0};    outrootTr->Branch("Nu",    &Nu,     "Nu/F");
  Float_t x{0};     outrootTr->Branch("x",     &x,      "x/F");
  Float_t y{0};     outrootTr->Branch("y",     &y,      "y/F");
  Float_t beamE{0}; outrootTr->Branch("beamE", &BeamEn, "beamE/F");
  // - electron kinematics branches
  Float_t eleE{0};             outrootTr->Branch("eleE",       &eleE,       "eleE/F");
  Float_t eleP{0};             outrootTr->Branch("eleP",       &eleP,       "eleP/F");
  Float_t elePt{0};            outrootTr->Branch("elePt",      &elePt,      "elePt/F");
  Float_t eleEta{0};           outrootTr->Branch("eleEta",     &eleEta,     "eleEta/F");
  Float_t elePhi{0};           outrootTr->Branch("elePhi",     &elePhi,     "elePhi/F");
  Float_t eleVertex[3]{0,0,0}; outrootTr->Branch("eleVertex",  eleVertex,   "eleVertex[3]/F");
  Int_t   eleStatus{0};        outrootTr->Branch("eleStatus",  &eleStatus,  "eleStatus/I");
  Float_t eleChi2pid{0};       outrootTr->Branch("eleChi2pid", &eleChi2pid, "eleChi2pid/F");
  Bool_t  eleFiduCut{false};   outrootTr->Branch("eleFiduCut", &eleFiduCut, "eleFiduCut/O");
  Float_t elePCALen{0};        outrootTr->Branch("elePCALen",  &elePCALen,  "elePCALen/F");
  Float_t eleECINen{0};        outrootTr->Branch("eleECINen",  &eleECINen,  "eleECINen/F");
  Float_t eleECOUTen{0};       outrootTr->Branch("eleECOUTen", &eleECOUTen, "eleECOUTen/F");
  Int_t   eleSector{0};        outrootTr->Branch("eleSector",  &eleSector,  "eleSector/I");
  // - hadron branches
  Int_t   pairType{0};                      outrootTr->Branch("pairType",   &pairType,  "pairType/I");
  Int_t   hadRow[2]{0,0};                   outrootTr->Branch("hadRow",     hadRow,     "hadRow[2]/I");
  Int_t   hadIdx[2]{0,0};                   outrootTr->Branch("hadIdx",     hadIdx,     "hadIdx[2]/I");
  Float_t hadE[2]{0,0};                     outrootTr->Branch("hadE",       hadE,       "hadE[2]/F");
  Float_t hadP[2]{0,0};                     outrootTr->Branch("hadP",       hadP,       "hadP[2]/F");
  Float_t hadPt[2]{0,0};                    outrootTr->Branch("hadPt",      hadPt,      "hadPt[2]/F");
  Float_t hadEta[2]{0,0};                   outrootTr->Branch("hadEta",     hadEta,     "hadEta[2]/F");
  Float_t hadPhi[2]{0,0};                   outrootTr->Branch("hadPhi",     hadPhi,     "hadPhi[2]/F");
  Float_t hadXF[2]{0,0};                    outrootTr->Branch("hadXF",      hadXF,      "hadXF[2]/F");
  Float_t hadVertex[2][3]{{0,0,0},{0,0,0}}; outrootTr->Branch("hadVertex",  hadVertex,  "hadVertex[2][3]/F");
  Int_t   hadStatus[2]{0,0};                outrootTr->Branch("hadStatus",  hadStatus,  "hadStatus[2]/I");
  Float_t hadBeta[2]{0,0};                  outrootTr->Branch("hadBeta",    hadBeta,    "hadBeta[2]/F");
  Float_t hadChi2pid[2]{0,0};               outrootTr->Branch("hadChi2pid", hadChi2pid, "hadChi2pid[2]/F");
  Bool_t  hadFiduCut[2]{false,false};       outrootTr->Branch("hadFiduCut", hadFiduCut, "hadFiduCut[2]/O");
  // - dihadron branches
  Float_t Mh{0};      outrootTr->Branch("Mh",      &Mh,      "Mh/F");
  Float_t Mmiss{0};   outrootTr->Branch("Mmiss",   &Mmiss,   "Mmiss/F");
  Float_t Z[2]{0,0};  outrootTr->Branch("Z",       Z,        "Z[2]/F");
  Float_t Zpair{0};   outrootTr->Branch("Zpair",   &Zpair,   "Zpair/F");
  Float_t xF{0};      outrootTr->Branch("xF",      &xF,      "xF/F");
  Float_t alpha{0};   outrootTr->Branch("alpha",   &alpha,   "alpha/F");
  Float_t theta{0};   outrootTr->Branch("theta",   &theta,   "theta/F");
  Float_t thetaLI{0}; outrootTr->Branch("thetaLI", &thetaLI, "thetaLI/F");
  Float_t zeta{0};    outrootTr->Branch("zeta",    &zeta,    "zeta/F");
  Float_t Ph{0};      outrootTr->Branch("Ph",      &Ph,      "Ph/F");
  Float_t PhPerp{0};  outrootTr->Branch("PhPerp",  &PhPerp,  "PhPerp/F");
  Float_t PhEta{0};   outrootTr->Branch("PhEta",   &PhEta,   "PhEta/F");
  Float_t PhPhi{0};   outrootTr->Branch("PhPhi",   &PhPhi,   "PhPhi/F");
  Float_t R{0};       outrootTr->Branch("R",       &R,       "R/F");
  Float_t RPerp{0};   outrootTr->Branch("RPerp",   &RPerp,   "RPerp/F");
  Float_t RT{0};      outrootTr->Branch("RT",      &RT,      "RT/F");
  Float_t PhiH{0};    outrootTr->Branch("PhiH",    &PhiH,    "PhiH/F");
  // -- phiR angles
  Float_t PhiRq{0};   outrootTr->Branch("PhiRq",   &PhiRq,   "PhiRq/F");
  Float_t PhiRp{0};   outrootTr->Branch("PhiRp",   &PhiRp,   "PhiRp/F");
  Float_t PhiRp_r{0}; outrootTr->Branch("PhiRp_r", &PhiRp_r, "PhiRp_r/F");
  Float_t PhiRp_g{0}; outrootTr->Branch("PhiRp_g", &PhiRp_g, "PhiRp_g/F");
  // -- SDME kinematics
  Float_t sdmePhiU{0}; outrootTr->Branch("sdmePhiU", &sdmePhiU, "sdmePhiU/F");
  Float_t sdmePhiL{0}; outrootTr->Branch("sdmePhiL", &sdmePhiL, "sdmePhiL/F");
  // - event-level branches
  Int_t runnum{0};   outrootTr->Branch("runnum",   &runnum,   "runnum/I");
