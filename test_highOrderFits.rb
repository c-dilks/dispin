...
# - main purpose is for testing higher order partial waves (L>2)
...
fit = Proc.new do |lmax|
  bruArgs = [
    "bruspin.volatile/lmaxtest.#{minimizer}.#{ivString}.lmax#{lmax}",
    minimizer,
    "",
    ivType,
    *nbins,
...
lmaxSeq.each{ |i| fit.call i } # loop
jobFile.close
...
if slurm
  ...
  slurmSet.call("array",         "1-#{lmaxSeq.length}")
