...
void FiducialCuts::ApplyCuts(int runnum_, int pid_, int evnum_) {
  ...
  if(pid_==11) { // electrons
    ...
    for(int r=0; r<nReg; r++) {
      fcutEleDC[r] = part_DC_Traj_found[0]>0 ? this->DC_fiducial_cut_XY(0, r+1, pid_) : false;
    };
    fiduCut = fcutElePCAL && fcutEleDC[0] && fcutEleDC[1] && fcutEleDC[2];
  }
  else if(pid_==211 || pid_==-211 || pid_==2212) { // pions and protons
    // inbending:  DC cuts on chi2/NDF, using polynomial border in (theta,phi) plane
    // outbending: DC cuts on chi2/NDF, using straight lines on xy hit plane
    for(int r=0; r<nReg; r++) {
      switch(torus) {
        case kInbending:  fcutHadDC[r] = part_DC_Traj_found[0]>0 ? this->DC_fiducial_cut_theta_phi(0,r+1,pid_) : false; break;
        case kOutbending: fcutHadDC[r] = part_DC_Traj_found[0]>0 ? this->DC_fiducial_cut_XY(0,r+1,pid_)        : false; break;
      };
    };
