...
Bool_t EventTree::Valid() {
  // return cutXCHECK; // test XCHECK CUTS //////////////////////
...
Float_t EventTree::GetDepolarizationFactor(Char_t kf) {
  ...
  if(useXCHECKdefinitions) { // XCHECK MODIFIED FACTORS //////////////////////////////
    ...
    if(kf=='A')      return dfA * iFactor;
    else if(kf=='W') return dfA * TMath::Sqrt(2*epsilon*(1-epsilon)) * iFactor/TMath::Sqrt(1+gamma*gamma);
    // other depol factors not (yet) modified:
  ...
  else { // original definitions used in PRL
    if(kf=='A')      return dfA;
    else if(kf=='B') return dfA * epsilon;
    else if(kf=='C') return dfA * TMath::Sqrt(1-epsilon*epsilon);
    else if(kf=='V') return dfA * TMath::Sqrt(2*epsilon*(1+epsilon));
