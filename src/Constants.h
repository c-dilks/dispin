...
static Int_t RundepTorus(Int_t run, TString fileName="") {
  ...
  else if(run==16333 || run==16771) return kInbending;
...
static Float_t RundepBeamEn(Int_t run) {
  ...
  else if(run==16333 || run==16771) return 10.5473;
...
static Bool_t RundepHelicityFlip(Int_t run) {
  ...
  else if(run==16333 || run==16771) return false;
  else {
...
static Float_t RundepPolarization(Int_t run, Bool_t v=true) {
  ...
  /* RGC test */
  else if(run==16333 || run==16771) return v? 0.80 : 0.02; // just a guess
  /* MC */
