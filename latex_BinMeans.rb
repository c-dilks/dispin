...
NumRows = 4 # try to keep this number equal to the number of higher dimensional bins, or a multiple thereof
looper.binHash.each do |ivType,binOpts|

  # add subsection
  subsectionT = "Bins of #{binOpts[:xTitle].gsub(/ \[.*$/,'')}"
  subsectionT += ", #{binOpts[:blTitle].gsub(/ __BL__/,'')}" unless binOpts[:blTitle].nil?
  out.puts "\\subsection{#{subsectionT}}\\label{sec:meanvmean:#{ivType}}"
  ...
  toriPngs.inject(:zip).map(&:flatten).each do |pngs|
    ...
    # header
    if row%NumRows==0
      placement = row/NumRows==0 ? 'h' : 'p'
      out.puts "\\begin{figure}[#{placement}]"
      out.puts "\\centering"
    end
    ...
    if row%NumRows==NumRows-1 or row==numPngs-1
      ...
      caption.sub!(/\.$/,", with each pair of rows corresponding to the two #{highDimVar} bins.") if highDim
