// CompareDiagDiphDists ------------------------------
// draw kinematic distributions from diagdiph/cat*.root files
...
void CompareDiagDiphDists(
  TString infile0N="diagdiph/cat__outroot.rga_inbending_fa18__1__0x3b.root", /*green*/
  TString infile1N="diagdiph/cat__outroot.rga_inbending_sp19__1__0x3b.root"  /*purple*/
) {
  infile[0] = new TFile(infile0N,"READ");
  infile[1] = new TFile(infile1N,"READ");
  for(f=0;f<2;f++) tr[f] = (TTree*)infile[f]->Get("diphTr");


  gStyle->SetOptStat(0);
  gStyle->SetPalette(kGreenPink);
  gROOT->ProcessLine(".! mkdir -p diagdiph/comp");
...
void CompareDist(TString varname) {
  ...
  rat->Draw("E");
  canv->Print("diagdiph/comp/"+TString(varname)+".png");
