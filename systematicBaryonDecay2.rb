...
plotPoints.each do |bl,points|
  ...
  makePlot = Proc.new do |ax,ySym,ySymErr,color|
    ax.errorbar(
      ...
      points[ySym],
      yerr:       points[ySymErr],
      marker:     inFileN.include?('22gev')?'s':'o',
      color:      color,
      ecolor:     color,
      linestyle:  'None',
      elinewidth: 2,
    ...
    axTitle = "Fraction from $#{pidLut[specificParentPID][:name]}$"
    axTitle += " - $#{ivName[1]}$ Bin #{bl+1}" if blList.length>1
    ax.set_title  axTitle
    ax.set_xlabel "$#{ivName[0]}$"
  end

  makePlot.call axHash[:fBaryon],   :fBaryon,   :fBaryonErr,   inFileN.include?('22gev')?'r':'b'
  # makePlot.call axHash[:aluSysUnc], :aluSysUnc, :aluSysUncErr, 'r'
  
  axHash[:fBaryon].set_ylim   0.0, 0.8
  # axHash[:aluSysUnc].set_ylim 0.0, 0.8
  # axHash[:fBaryon].set_ylabel   'fraction'
  # axHash[:aluSysUnc].set_ylabel '$\Delta_B$'

  if ivName[0].match? /^M/
    axHash[:fBaryon].set_xlim 0.2, 1.5
  elsif ivName[0].match? /^P/
    axHash[:fBaryon].set_xlim 0.1, 1.0
  elsif ivName[0].match? /^x/
    axHash[:fBaryon].set_xlim 0.04, 0.6
  elsif ivName[0].match? /^z/
    axHash[:fBaryon].set_xlim 0.1, 0.9
  end
...
##
plt.savefig("#{outFilePrefix}.png", transparent: true)
